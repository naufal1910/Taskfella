...
import { assertTaskInProject, permanentlyDeleteTask } from "@/server/modules/tasks/service";
...
export async function POST(request: Request, context: Context): Promise<NextResponse> {
  ...
  return taskRoute(
    ...
    async ({ account }) => {
      ...
      await permanentlyDeleteTask(getDatabase(), account.id, taskId, body.confirmation);
      return taskJson({ ok: true });
    },
    true,
  );
}
