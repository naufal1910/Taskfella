...
Open <http://localhost:3000>. After signing in, open the project/task workspace at <http://localhost:3000/projects>. Check database and application readiness with:
...
The local code-quality pass is `pnpm validate`; the full Phase 2 and Phase 3 verification sequence, including migration, readiness, transactional integration, responsive board checks, accessibility, and build checks, is in the [Phase 3 verification record](docs/implementation/taskfella-phase3-tasks.md). Database-backed tests require the local database to be running and migrated. Compose exposes PostgreSQL on host port `5433` so it can coexist with other local projects; `.env.example` matches that port. See the [foundation operations guide](docs/operations.md) for authentication delivery, local message artifacts, trusted-proxy rate limits, and production SMTP configuration.
...
- [Phase 0 through Phase 3 documentation index](docs/README.md)
...
- [Phase 3 tasks and board execution issue](https://github.com/naufal1910/Taskfella/issues/5)
...
- [Phase 3 tasks and board execution](docs/implementation/taskfella-phase3-tasks.md)
