...
The application serializes project workflow changes with a PostgreSQL advisory transaction lock plus a row lock. Revision preconditions turn stale client edits into `CONCURRENT_UPDATE` conflicts. Role changes crossing the completed boundary require `confirmCompletionChanges`; invalid final role sets are rejected before any update is committed. Column deletion is permitted only for an empty Phase 2 workflow column and is still checked against the role invariants.

`src/server/modules/workflow/wip.ts` owns the WIP policy. `none` always allows, `warn` requires an explicit confirmation retry when the authoritative count is at the limit, and `enforce` rejects overflow. `assertColumnWip` runs the count reader inside the project-locked transaction so a future task move cannot authorize from a stale browser count. No client-only WIP warning is treated as authoritative.
...
Executable coverage includes template validity, account isolation, workflow ordering and configuration, role confirmation, WIP modes, deferred database invariant rejection, concurrent revision serialization, archive/restore retention, explicit destructive deletion, migration/readiness behavior, and responsive/accessibility browser checks at desktop and 390px mobile dimensions using local fixtures only. On 2026-08-22, `chrome-devtools-axi` verified the authenticated project list and board at 1440px and 390px: desktop multi-column navigation, mobile one-column selection, bottom navigation, typed destructive confirmation, workflow dialog semantics, and visible focus/labels. Lighthouse reported Accessibility 100, Best Practices 100, and SEO 100 for the local project list.
